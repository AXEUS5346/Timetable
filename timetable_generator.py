...
def main():
    ...
    with st.sidebar:
        ...
        # Rules document
        rules_doc_col1, rules_doc_col2 = st.columns([3, 1])
        with rules_doc_col1:
            rules_file = st.file_uploader(
                "Upload Rules Document",
                type=["md", "txt", "csv"],
                help="Upload a markdown or CSV file containing the rules for timetable creation"
            )
        with rules_doc_col2:
            if rules_file:
                if st.button("Clear Rules", key="clear_rules"):
                    st.session_state.rules_retriever = None
                    st.rerun()

        if rules_file:
            with st.expander("Preview Rules Document", expanded=False):
                try:
                    # For CSV files
                    ...
                    # For text and markdown files
                        ...
                        content = rules_file.getvalue().decode('utf-8')
                        st.markdown(content)
                except Exception as e:
        ...
        if rules_file and st.session_state.llm:
            ...
            elif st.button("Process Rules", type="primary"):
        ...
        # Organization info document (same structure as rules document)
        org_info_doc_col1, org_info_doc_col2 = st.columns([3, 1])
        with org_info_doc_col1:
            org_info_file = st.file_uploader(
                "Upload Organization Information",
                type=["md", "txt", "csv"],
                help="Upload a markdown or CSV file containing information about courses, faculty, etc."
            )
        with org_info_doc_col2:
            if org_info_file:
                if st.button("Clear Org Info", key="clear_org_info"):
                    st.session_state.org_info_retriever = None
                    st.rerun()

        if org_info_file:
            with st.expander("Preview Organization Info", expanded=False):
                try:
                    # For CSV files
                    if org_info_file.name.endswith('.csv'):
                        df = pd.read_csv(org_info_file)
                        st.dataframe(df)
                    # For text and markdown files
                    else:
                        content = org_info_file.getvalue().decode('utf-8')
                        st.markdown(content)
                except Exception as e:
                    st.error(f"Error previewing file: {str(e)}")
            ...
            if st.session_state.llm:
                if st.session_state.embedding_model is None:
                    st.warning("⚠️ Please initialize the embedding model before processing documents")
                elif st.button("Process Organization Info", type="primary"):
            ...
            with st.chat_message("assistant"):
                ...
                try:
                    with st.spinner("Generating response..."):
                        ...
                        {rules_context}
                        ALWAYS check organization information before responding.
